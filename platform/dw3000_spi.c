...
#define DW_CS_PSEL                                                             \
	(DT_PROP(DT_SPI_DEV_CS_GPIOS_CTLR(DW_INST), port) * 32U                    \
	 + DT_SPI_DEV_CS_GPIOS_PIN(DW_INST))

BUILD_ASSERT(DT_SPI_DEV_CS_GPIOS_FLAGS(DW_INST) & GPIO_ACTIVE_LOW,
			 "DW3000 chip select must be active low");
